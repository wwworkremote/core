...
class DatabaseBackupJob < ApplicationJob
  ...
  def perform
    ...
    begin
      ...
      system("pg_dump -Fc -d #{url} -f #{full_backup}")
      
      # 2. Selective Critical Backup (User Identity, History, Queries, and Pipelines)
      # These tables contain the data that is hardest to re-create.
      critical_tables = %w[
      ...
      table_flags = critical_tables.map { |t| "-t #{t}" }.join(" ")
      system("pg_dump -Fc -d #{url} #{table_flags} -f #{critical_backup}")
