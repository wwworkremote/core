  ...
  class LinkMonitorJob < ApplicationJob
    ...
    def check_link(job)
      ...
      if response.status == 404
        ...
        archive_job(job, 'Job no longer available (Redirect signal)') if follow_up.body.downcase.include?('job is no longer available') || follow_up.body.downcase.include?('position has been filled')
      end
