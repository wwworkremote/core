...
class UserJobPostingsController < ApplicationController
  ...
  def create
    ...
    redirect_back fallback_location: job_posting_path(@job_posting), notice: "Job status updated."
  end

  def update
    @user_job_posting = current_user.user_job_postings.find(params[:id])
    if @user_job_posting.update(user_job_posting_params)
      redirect_back fallback_location: user_job_postings_path, notice: "Job record updated."
    end
  end
  ...
  def destroy
    ...
    redirect_back fallback_location: user_job_postings_path, notice: "Job removed from your list."
  end
  ...
  def user_job_posting_params
    params.require(:user_job_posting).permit(:status, :notes)
  end
end
