...
class OutboundLinksController < ApplicationController
  def show
    ...
    if job_posting&.target_url.present?
      ...
      verified_url = JobPosting.where(target_url: params[:url]).limit(1).pluck(:target_url).first
      ...
      if verified_url.present?
        ...
        redirect_back fallback_location: root_path, alert: "Security Exception: Invalid destination."
      end
    end
  end
end
