...
module Admin
  class JobPostingsController < Admin::ApplicationController
    def index
      ...
      if @status == 'purged'
        @job_postings = @job_postings.where(status: 'purged')
      else
        @job_postings = @job_postings.where.not(status: 'purged')
      end
    ...
    def purge
      @job_posting = JobPosting.find(params[:id])
      @job_posting.purge!
      redirect_back fallback_location: admin_job_postings_path, notice: "Job record moved to trash and vectors cleared."
    end

    def restore
      @job_posting = JobPosting.find(params[:id])
      @job_posting.restore!
      redirect_back fallback_location: admin_job_postings_path, notice: "Job record restored to registry."
    end
    ...
    def bulk_action
      ...
      redirect_back fallback_location: admin_job_postings_path, notice: notice || "No records selected."
    end
