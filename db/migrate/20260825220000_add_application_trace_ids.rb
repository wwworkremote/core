...
class AddApplicationTraceIds < ActiveRecord::Migration[7.1]
  ...
  def change
    add_column :user_job_postings, :application_trace_id, :string unless column_exists?(:user_job_postings,
                                                                                        :application_trace_id)
    add_index :user_job_postings, :application_trace_id, algorithm: :concurrently unless index_exists?(
      :user_job_postings, :application_trace_id
    )
    ...
    add_index :extension_error_events, :trace_id, algorithm: :concurrently unless index_exists?(
      :extension_error_events, :trace_id
    )
  end
end
