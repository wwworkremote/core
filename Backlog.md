...
- The supported service controller is `bin/wwwr-ctl`, with launchd-backed `start`, `stop`, `restart`, and `status` commands.
- Rails development host authorization now allows `lan.wwworkremote.com` and loopback addresses.
- `bin/wwwr-ctl start/restart web` now waits up to 30 seconds for Puma to answer HTTP requests.
...
- `bin/wwwr-ctl restart all` returns only after web readiness is confirmed, or clearly reports a bounded startup failure.
