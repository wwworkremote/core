...
ActiveSupport::Notifications.monotonic_subscribe('request.faraday') do |event|
  ...
  context = Rails.configuration.x.context.merge(signature:)

  begin
    next if Source.exists?(signature:)

    Source.create(signature:, payload:, event: event_json)
    ...
    Sentry.capture_exception(e)
    Rails.logger.error(e.message, context.merge(signature:))
