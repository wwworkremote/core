...
Use the `interview-prep` skill when an interview is scheduled or Mike wants a briefing sharpened. It generates a prep pack (`bin/wwwr interview-prep <job_posting_id> [--regenerate]` → `LLM::InterviewPrepGenerator`, stored on `UserJobPosting#interview_prep_pack`,  and can export both versions to `~/ai/outbox/wwwr/interview-prep/<role>/` for text-to-speech (`--export`), then audits and grounds it: the `interview-prep-auditor` subagent diffs the pack against the quality bar (`docs/interview-prep/_reference/reference.md`) and returns a mechanical/judgement punch list, and `industry-intelligence-agent` verifies the domain primer's concepts, acronym-to-authority citations, and learning links against real sources. The local generator's structure is reliable but its content is a scaffold — it mixes Mike's own stack into the primer's "useful context", files skill gaps as domain blind spots, and sometimes invents a link; a stronger model on that path is blocked on TASK-145. The pack ships in two versions (human + read-aloud text-to-speech); `bin/wwwr interview-prep <id> --export` writes both to `~/ai/outbox/wwwr/interview-prep/<role>/`. Tooling diagrams: `docs/architecture/interview-prep-tooling.md`.
