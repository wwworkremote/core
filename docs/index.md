...
- **Metaphor Harmony & Domain Cartography**: `.claude/agents/domain-cartographer-agent.md` and `.claude/skills/metaphor-harmony/SKILL.md` for auditing domain vocabulary and conceptual resonance.

### CLI Help & Manual
- `man wwwr` (or `man ./man/wwwr.1`) — complete troff manual for the `bin/wwwr` operational CLI.
- `bin/wwwr --help` — pipeline status, browse postings, trigger transitions, match scoring, interview-prep generation.
- `bin/wwwr-ctl status` — service controller for web and background jobs LaunchAgents.
