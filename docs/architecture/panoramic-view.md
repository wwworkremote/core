...
"Panoramic View" is a **technique**: the system-cartography investigation method. Per
platform naming conventions, it is never an initiative or brand name (Panoramic View Labs /
PVL is the initiative umbrella, Pavel is the specialist agent, and zdots is the root platform).
wwworkremote borrows the *technique* and applies it locally to application traces, with no
dependency on external platforms.

The technique has two prior implementations, and the difference between them is the whole design
lesson:
...
- **Vertical = depth through the topology per step**, not a generic call stack — the same "lanes"
  ...
  *(Harmonization note: ADR 004 rejected a sinusoidal model for the macro workflow loop because
  that implied passive oscillation without agency or momentum; here, the wave models the
  micro spatial propagation through architectural layers per round-trip).*
