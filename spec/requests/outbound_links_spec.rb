...
RSpec.describe "Outbound Links" do
  ...
  describe "GET /outbound_link" do
    it "redirects to the job's target URL with wwwr_id appended, and tracks the event" do
      get outbound_link_path(id: "tracking", job_posting_id: job.id)

      expect(response).to redirect_to("https://trusted-site.com/apply?wwwr_id=#{job.id}")
    end
