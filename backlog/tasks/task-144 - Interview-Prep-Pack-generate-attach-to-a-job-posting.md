...
references:
  ...
  - 'https://jobs.lever.co/[redacted]/[redacted]'
modified_files:
...
Prompted by a real [redacted] (Lever) interview on 2026-09-03.
...
Verification: db:migrate; rspec the 3 files; rubocop the new files; manual against [redacted] posting (UJP #268) — generate/edit/persist/regenerate; confirm write via runner.
...
Live generation against the local model was NOT exercised -- Ollama was not running during this session (orchestrator returned connection errors). The generation path is byte-for-byte the ArtifactGenerator pattern (proven in production) with the orchestrator stubbed in specs. Prompt assembly WAS run against the real [redacted] posting (JP #7068 / UJP #268): title, company_name=[redacted], 5563-char body, 12 ranked experiences, prior match_analysis, standing criteria, and the 'omit the referral section' line (no Contact linked) all present, 31k chars total, within the local model's 32k context.
...
UPDATE 2026-09-02: live end-to-end run completed. The local model server (llama.cpp on :11500, provider `local`) was actually up. Generated a real ~9k-char pack for a real posting (UJP #268) -- all 7 sections, referral section correctly omitted ('Section omitted as no referral is listed'), comp reality check present (posted band vs the configured floor), grounded in real history (Rails, OpenTelemetry, legacy modernization, prompt-injection defense, ADRs), no fabricated titles/employers. Verified rendering on the live posting page: markdown sections, 'Generated N ago', copy + Regenerate + Edit form all present.

Quality on the local 7B: usable scaffold, weaker than the hand-written reference -- generic in places, invents minor team detail not in the posting, undersells the React experience, no specific project names ([redacted]/CookieOverflow/ReachLocal). This is the signal the models.yml comment predicts for pointing defaults.interview_prep at a stronger model -- blocked on TASK-145 (LLM::Orchestrator fails on the Gemini provider, filed this session).
<!-- SECTION:NOTES:END -->
...
- **Live generation not exercised** — Ollama was down this session. Path is identical to the production ArtifactGenerator; prompt assembly verified against the real [redacted] posting. First real run should sanity-check output quality on the local 7B model; if thin, set `defaults.interview_prep` in `config/models.yml`.
- No `Contact` is linked to the [redacted] posting, so its pack will omit the referral section — Mike can add "[redacted]" via the existing contacts form on the posting page to get that section.
