...
- Numbers, money, ranges as words or spoken digits: "140,000 to 175,000 dollars" not "$[redacted]"; "under 100 milliseconds" not "sub-100ms"; "99th percentile" not "p99"; "4 percent" not "4%".
...
- [x] #7 The [redacted] pack (UJP #268) is regenerated and reads cleanly aloud
...
12. Regenerate the [redacted] pack -> both columns populated; spot-check the spoken one with `say`.
...
AC#8 pending: [redacted] pack regeneration running (2 LLM calls on the local model).

Docs regrouped under docs/interview-prep/ (commit 93fa21e1): _reference/reference.md, tts-readable-documentation.md, tts-transform-prompt.md, plus a README. One per-role subdirectory. AC#5's 'rewrite interview-prep-[redacted]-dsp.md TTS-clean' stays a deliberate no -- it is a human reference doc, not a speech source; it points to the standard instead.
